    ...
    >
      <div ref={ref} className="max-w-3xl mx-auto p-6 lg:pt-0 space-y-6">
        ...
        <div className="relative w-full aspect-[325/202] mt-4 border-2 border-gray-110 rounded-3xl overflow-clip">
          <Image
            src={ExpenseTrackerPortfolio.projectImage}
            alt={ExpenseTrackerPortfolio.projectName}
            width={3000}
            height={2000}
            quality={100}
          />
        ...
        <h1 className="font-bold text-5xl">
          {ExpenseTrackerPortfolio.projectName}
        </h1>
        <p className="text-lg text-gray-140">
          {ExpenseTrackerPortfolio.overview}
        </p>
        ...
        <div>
          ...
          <p className="text-lg text-gray-140 pt-2">
            {ExpenseTrackerPortfolio.issues}
          </p>
        </div>
        ...
        <div>
          ...
          <p className="text-lg text-gray-140 py-2">
            {ExpenseTrackerPortfolio.solutionsHeader}
          </p>

          {ExpenseTrackerPortfolio.solutionsList.map((solution, index) => (
          ...
          <p className="text-lg text-gray-140 pt-2">
            {ExpenseTrackerPortfolio.solutionsConclusion}
          </p>
        </div>

        <TechStack
          title="Tech Stack (Frontend & Backend Stack)"
          techStackTool={ExpenseTrackerPortfolio.techstack.fenbe}
        ></TechStack>

        <TechStack
          title="Tech Stack (DevOps)"
          techStackTool={ExpenseTrackerPortfolio.techstack.devops}
        ></TechStack>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {ExpenseTrackerPortfolio.sections.featurenhighlights.items.map(
            (item, index) => (
              <div
                key={index}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {ExpenseTrackerPortfolio.sections.impactandoutcomes.items.map(
            (item, index) => (
              <div
                key={index}
                className="flex  flex-col gap-4 bg-gray-110  rounded-lg p-4"
