    ...
    >
      <div ref={ref} className="max-w-3xl mx-auto p-6 lg:pt-0 space-y-6">
        ...
        <div className="relative w-full aspect-[325/202] mt-4 border-2 border-gray-110 rounded-3xl overflow-clip">
          <Image
            src={NumazuScraperPortfolio.projectImage}
            alt={NumazuScraperPortfolio.projectName}
            width={3000}
            height={2000}
            quality={100}
          />
        ...
        <h1 className="font-bold text-5xl">
          {NumazuScraperPortfolio.projectName}
        </h1>
        <p className="text-lg text-gray-140">
          {NumazuScraperPortfolio.overview}
        </p>
        ...
        <div>
          ...
          <p className="text-lg text-gray-140 py-2">
            {NumazuScraperPortfolio.solutionsHeader}
          </p>

          {NumazuScraperPortfolio.solutionsList.map((solution, index) => (
          ...
          <p className="text-lg text-gray-140 pt-2">
            {NumazuScraperPortfolio.solutionsConclusion}
          </p>
        </div>

        <TechStack
          title="Tech Stack (Frontend & Backend Stack)"
          techStackTool={NumazuScraperPortfolio.techstack.fenbe}
        ></TechStack>

        <TechStack
          title="Tech Stack (DevOps)"
          techStackTool={NumazuScraperPortfolio.techstack.devops}
        ></TechStack>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {NumazuScraperPortfolio.sections.featurenhighlights.items.map(
            (item, index) => (
              <div
                key={index}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {NumazuScraperPortfolio.sections.impactandoutcomes.items.map(
            (item, index) => (
              <div
                key={index}
                className="flex  flex-col gap-4 bg-gray-110  rounded-lg p-4"
